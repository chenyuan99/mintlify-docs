...
export const InfoLog = ({ message, detail }) => {
    if (process.env.CLI_TEST_MODE === 'true') {
        console.log(`info ${message}${detail ? ` ${detail}` : ''}`);
        return null;
    }
    return (_jsxs(Box, { children: [_jsx(Text, { color: "blue", bold: true, children: "info" }), _jsxs(Text, { children: [" ", message] }), detail && _jsxs(Text, { dimColor: true, children: [" ", detail] })] }));
};
...
export const BrokenLinksLog = ({ brokenLinksByFile, }) => {
    ...
    if (process.env.CLI_TEST_MODE === 'true') {
        ...
        for (const [filename, brokenLinks] of Object.entries(brokenLinksByFile)) {
            console.log(filename);
            brokenLinks.forEach((link, i) => {
                const prefix = i === brokenLinks.length - 1 ? '└─' : '├─';
