...
/**
 * Returns the FileSchema contentEncoding when a string schema represents a file upload.
 * Covers OpenAPI 3.0 `format: binary|base64` and OpenAPI 3.1 `contentMediaType`
 * representations (with optional `contentEncoding` to choose base64 vs binary).
 *
 * `contentMediaType` alone only counts as a file for known binary media-type families
 * (octet-stream, image/audio/video, pdf, common archives). `contentEncoding` alone is
 * not enough — in JSON Schema it often means an inline encoded string, not an upload.
 */
export function getFileLikeStringEncoding(schema) {
    if (schema.format === 'binary' || schema.format === 'base64') {
        return schema.format;
    }
    if (typeof schema.contentMediaType !== 'string' ||
        !isFileLikeContentMediaType(schema.contentMediaType)) {
        return undefined;
    }
    if (schema.contentEncoding === 'base64' || schema.contentEncoding === 'base64url') {
        return 'base64';
    }
    return 'binary';
}
const FILE_LIKE_APPLICATION_MEDIA_TYPES = new Set([
...
function isFileLikeContentMediaType(contentMediaType) {
    var _a, _b;
    const normalized = contentMediaType.toLowerCase().trim();
    if (normalized.length === 0) {
        return false;
    }
    // Strip optional parameters (e.g. "image/png; charset=binary")
    const mediaType = (_b = (_a = normalized.split(';', 1)[0]) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : normalized;
    return (mediaType.startsWith('image/') ||
        mediaType.startsWith('audio/') ||
        mediaType.startsWith('video/') ||
        FILE_LIKE_APPLICATION_MEDIA_TYPES.has(mediaType));
}
/** Read OpenAPI 3.1 JSON Schema file keywords (not always present on openapi-types). */
export function getSchemaContentMediaType(schema) {
    if (!('contentMediaType' in schema)) {
        return undefined;
    }
    const contentMediaType = schema.contentMediaType;
    return typeof contentMediaType === 'string' ? contentMediaType : undefined;
}
export function getSchemaContentEncoding(schema) {
    if (!('contentEncoding' in schema)) {
        return undefined;
    }
    const contentEncoding = schema.contentEncoding;
    return typeof contentEncoding === 'string' ? contentEncoding : undefined;
}
...
export function copyAndCombineKeys(acc, curr) {
    ...
    copyKeyIfDefined('contentMediaType', curr, acc);
    copyKeyIfDefined('contentEncoding', curr, acc);
