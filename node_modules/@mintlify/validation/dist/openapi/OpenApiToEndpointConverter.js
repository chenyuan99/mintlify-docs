...
import { parseXMcp, parseXMint } from './parseXMint.js';
...
// TODO: Delete this file once its last consumer is removed: legacy Trieve search
// indexing in server (`generateOpenApiChunk` in api/utils/search/openapi.ts — see
// the TODO there). Nothing else uses these converters. When deleting, also remove
// the `export *` in src/index.ts, __test__/openapi/OpenApiToEndpointConverter.test.ts,
// and any openapi/ helper converters that become unused.
export class BaseOpenApiToEndpointConverter extends BaseConverter {
    ...
    getXmcp(operation) {
        if ('x-mcp' in operation &&
            ...
            return parseXMcp(operation['x-mcp']);
        }
        return undefined;
    }
    getXmint(operation) {
        if ('x-mint' in operation &&
            ...
            return parseXMint(operation['x-mint']);
        }
        return undefined;
    }
}
