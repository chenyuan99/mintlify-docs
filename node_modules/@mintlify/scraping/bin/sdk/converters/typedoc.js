    ...
    for (const module of modules) {
        ...
        for (const section of SECTIONS) {
            ...
            for (const entry of entries) {
                if (entry.node.kind !== KIND.class && entry.node.kind !== KIND.interface)
                    continue;
                const parentSlug = slugById.get(entry.node.id) ??
                    path.posix.join(multiModule ? slugify(module.name) : '', ...entry.namespacePath.map(slugify), section.dir, slugify(entry.node.name));
...
class TypedocRenderer {
    ...
    renderMemberPages(parent, parentSlug, parentTitle) {
        const members = (parent.children ?? []).filter((child) => !child.flags?.isPrivate);
        const pages = [];
        for (const property of members.filter((child) => child.kind === KIND.property || child.kind === KIND.accessor)) {
