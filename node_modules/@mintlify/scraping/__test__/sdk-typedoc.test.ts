...
import { renderSdkTarget } from '../src/sdk/findSdkPage.js';
...
describe('convertTypedoc', () => {
  ...
  it('converts a typedoc project into pages and groups', async () => {
    ...
    const login = reference.pages.find((page) => page.slug === 'classes/Client/login');
    expect(login).toMatchObject({
      title: 'Client.login',
      tag: 'METHOD',
      target: { kind: 'method', name: 'login', parent: 'Client' },
    });
    expect(login?.content).toContain('login(): Promise<void>');

    const token = reference.pages.find((page) => page.slug === 'classes/Client/token');
    expect(token).toMatchObject({
      title: 'Client.token',
      tag: 'PROPERTY',
      target: { kind: 'property', name: 'token', parent: 'Client' },
    });

    expect(renderSdkTarget(reference, { kind: 'class', name: 'Client' }).slug).toBe(
      'classes/Client'
    );
    expect(
      renderSdkTarget(reference, {
        kind: 'method',
        name: 'login',
        parent: 'Client',
      }).slug
    ).toBe('classes/Client/login');
  });
});
